...
class SimpleConfigurationFactory(BaseConfigurationFactory):
    ...
    def create_coding(self, id, name, coded_basetype, coded_category, coded_termination, coded_bit_length, coded_min_length, coded_max_length, compu_scale, compu_consts):
        ret = Coding(id, name, coded_basetype, coded_category, coded_termination, coded_bit_length, coded_min_length, coded_max_length, compu_scale, compu_consts)
        self.__codings__[name] = ret
        return ret
    ...
    def create_pdu(self, id, short_name, byte_length, pdu_type, signal_instances):
        assert (short_name not in self.__pdus__)
    ...
    def create_multiplex_pdu(self, id, short_name, byte_length, pdu_type, switch, seg_pos, pdu_instances,
        ...
        ret = MultiplexPDU(id, short_name, byte_length, pdu_type, switch, seg_pos, pdu_instances,
                           static_segs, static_pdu)
        self.__multiplexpdus__[short_name] = ret
        return ret
...
class Coding(BaseCoding):
    def str(self, indent):
        ret = indent * " "
        ret += f"Coding {self.__name__}\n"
...
class PDUInstance(BasePDUInstance):
    def str(self, indent):
        #Changed to try except, because a PDU might not have signals defined and thus is not in __pduo__
        try:
...
def main():
    ...
    print(f"Found the channels {conf_factory.__channels__.keys()} in the file")
    write_msgs_pack = True
    for key in conf_factory.__channels__.keys():
        print(f"Generating output directories for {key}")
        fwriter = FileWriter(args.filename, key)
        if write_msgs_pack:
            write_msgs_pack = False
            print("Generating Package for FlexRay messages")
            fwriter.write_msgspack(conf_factory)
        fwriter.write_content_to_file('parsed_content.txt', conf_factory)
        fwriter.write_interface(conf_factory)
